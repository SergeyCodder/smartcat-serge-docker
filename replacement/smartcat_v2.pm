...
sub run_smartcat_cli {
    ...
    $params .=
        ' --server=' . $self->{data}->{base_url} .
        ' --workspace=' . $self->{data}->{account_id} . # smarcat-cli uses 'token-id' instead of 'account-id'
        ' --api-token=' . $self->{data}->{token} .
        ' --project=' . $self->{data}->{project_id};

    $params .= ' --debug' if $self->{data}->{debug};

    return $self->run_cmd('smartcat ' . $params);
}
